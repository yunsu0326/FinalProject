...
import com.spring.app.domain.MessageVO;
...
public class WebsocketEchoHandler extends TextWebSocketHandler {
   ...
   public void afterConnectionEstablished(WebSocketSession wsession) throws Exception {
	   ...
	   //====> 웹채팅확인용 : 0a9f400f-69e8-aa88-b5ab-e210a48677ae님이 접속했습니다.
	   //====> 웹채팅확인용 : a2da3a99-0e9a-fb84-6bf8-b50645c6bf44님이 접속했습니다.
       ...
       */
        ...
        */
		   ...
		   EmployeesVO loginuser = (EmployeesVO)map.get("loginuser");  
...
// === 클라이언트가 웹소켓 서버로 메시지를 보냈을때의 Send 이벤트를 처리하기 ===
  ...
  public void handleTextMessage(WebSocketSession wsession, TextMessage message) throws Exception {
	  ...
	  //System.out.println("===> 웹채팅 확인용 : 로그인ID : " + loginuser.getUserid());
	  ...
	  */
      ...
      for(WebSocketSession webSocketSession : connectedUsers) {
    	  ...
    	  if("all".equals(messageVO.getType())) {
    		  // 채팅할 대상이 "전체" 일 경우
        	  // 메시지를 자기자신을 뺀 나머지 모든 사용자들에게 메시지를 보낸다.
    		  ...
    		  if( !wsession.getId().equals(webSocketSession.getId())) {
    			  ...
    			  webSocketSession.sendMessage( // style='display:none'
   ...
   public void afterConnectionClosed(WebSocketSession wsession, CloseStatus status) throws Exception {
      ...
      EmployeesVO loginuser = (EmployeesVO)map.get("loginuser");
       ...
       for (WebSocketSession webSocketSession : connectedUsers) {
           ...
           EmployeesVO loginuser2 = (EmployeesVO)map2.get("loginuser");  
          // "loginuser" 은 HttpSession에 저장된 키 값으로 로그인 되어진 사용자이다.
