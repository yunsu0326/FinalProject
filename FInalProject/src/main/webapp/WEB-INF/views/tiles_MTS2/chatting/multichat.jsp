...
*/
	$(document).ready(function(){
		...
		//alert("root : " + root);
		//root : 192.168.10.206:9090/board/chatting
		
		const wsUrl = "ws://"+root+"/multichatstart.gw";
		//alert(wsUrl);
        // 웹소켓통신을 하기위해서는 http:// 을 사용하는 것이 아니라 ws:// 을 사용해야 한다. 
		
		const websocket = new WebSocket(wsUrl);
       
        // 즉, const websocket = new WebSocket("ws://192.168.10.206:9090/board/chatting/multichatstart.action"); 이다.
	    ...
	    */
	    	...
	    	websocket.onopen = function(){
	    		 //alert("웹소켓 연결됨!!");
	    		 $("div#chatMessage").append("<div style='background-color: #cce0ff; display: inline-block; width: 100%; padding: 7px; text-align:center;  word-break: break-all;'>※공지사항※</div>");
	    		
	    		 $("div#chatMessage").append("<div style='background-color: #cce0ff; display: inline-block; width: 100%; padding: 7px; text-align:center; word-break: break-all;'>대화내용이 검정색으로 보이면 채팅에 참여한 모두에게 보여지는 것입니다.</div>");
	    		 
	    		 $("div#chatMessage").append("<div style='background-color: #cce0ff; display: inline-block; width: 100%; padding: 7px; text-align:center; word-break: break-all;'>대화내용이 붉은색으로 보이면 나에게만 보여지는 1:1 대화 입니다.</div>");
	    	...
	    	/*
	    		...
	    		messageObj.type = "all"; // messageObj.type = "all" 은 "1대 다 "채팅을 뜻하는 것이고, messageObj.type = "one" 은 "1대 1 "채팅을 뜻하는 것이라고 하겠다.(개발자마음)
	    		messageObj.to ="all"; // messageObj.to ="all"; 은 모두를 뜻하는 것이고, messageObj.to ="eomjh"; 이라면 eomjh 인 사람과 1대1 채팅(귓속말)을 뜻하는 것으로 하겠다
	    	*/
	    	// 또는
	    		messageObj = {message: "채팅방에<span style='color:red;'>입장</span> 했습니다."
	    					 ,type : "all"
	    	...
	    	websocket.onmessage = function(event){
	    		...
	    		// event.data 는 수신되어진 메시지이다. 즉, 지금은  「박윤수이순신」 이다.
	    		if(event.data.substr(0,1)=="「" && event.data.substr(event.data.length-1)=="」"){
	    			$("span#userList").text(event.data);
	    		}
	    		else{
	    	...
	    	$("input#btnSendMessage").click(function(){
	    		if($("input#message").val().trim() != ""){
                    ...
                    if(isOnlyOneDialog == false){ //귓속말이 아닌 경우
                    	$("div#chatMessage").append("<div style='background-color: rgb(3,199,90); display: inline-block; max-width: 60%; float: right; padding: 7px; border-radius: 15%; word-break: break-all;'>" + messageVal + "</div> <div style='display: inline-block; float: right; padding: 20px 5px 0 0; font-size: 7pt;'>"+currentTime+"</div> <div style='clear: both;'>&nbsp;</div>");
	        ...
	        */
	    		...
	    		$("button#btnAllDialog").show(); // 귓속말 대화끊기 버튼 보이기
	            $("input#message").css({'background-color':'#ffff99', 'color':'black'});
	            $("input#message").attr("placeholder","1:1대화 메시지 내용");
	...
	function closePop(){
		...
		var result = confirm("나가면 채팅내용이 모두 사라집니다. 나가시겠습니까?");
	      if (result) {
	        // 사용자가 확인을 선택한 경우
	        window.self.close(); // 또는 다른 팝업 닫는 방법을 사용할 수 있습니다.
	      } else {
...
<div class="container-fluid" style="height:890px;">
<div class="row">
<div class="col-md-10 offset-md-1">

   <div style="text-align:center;">
   	<span id="chatStatus" style="font-weight:bold; font-size:30px;"></span>
   </div>
   ...
   <div id="chatMessage" style="height: 600px; overFlow: auto; margin: 20px 0; background-color:#cce0ff;"></div>
	
	
   		<input type="text" style="width:85%; display:inline-block;"   id="message" class="form-control" placeholder="메시지 내용"/>
   		<input type="button" id="btnSendMessage" class="btn btn-md ml-2 mb-1" style="background-color:rgb(3,199,90); color:white; width:70px;" value="전송" />
		...
		<div style="display:flex; width:100%; margin-top:50px;">
			<div style="margin-right:auto;">
        		<span style="display:inline-block; font-weight:bold; font-size:20px; color:black;">1:1대화 대상 : </span>
        		<span id="privateWho" style="font-weight: bold; font-size:20px; color:black;"></span>
        		<button type="button" id="btnAllDialog" class="btn btn-sm" style="background-color:#663300; color:white; font-weight:bold;">1:1 대화끊기</button>
   			</div>
   			<div style="margin-left:auto;">
   				<input type="button" class="btn btn-sm mt-1" style="background-color:black; color:white; " onclick="closePop()" value="채팅방나가기" />
   			</div>
   		</div>
</div>  
