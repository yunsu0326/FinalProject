...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("select.calType").change(function(){
			var fk_lgcatgono = $("select.calType").val();      // 내캘린더이라면 1, 사내캘린더이라면 2 부서캘린더라면 3이다.
			var fk_employee_id = $("input[name=fk_employee_id]").val();  // 로그인 된 사용자아이디
			
			if(fk_lgcatgono != "") { // 선택하세요 가 아니라면
				$.ajax({
						url: "<%= ctxPath%>/schedule/selectSmallCategory.gw",
		...
		//	console.log("공유자 => " + joinuser);
			...
			frm.action="<%= ctxPath%>/schedule/registerSchedule_end.gw";
			frm.method="post";
			frm.submit();
...
<h3>일정 등록</h3>
	...
	<form name="scheduleFrm">
		...
		<input type="hidden" value="${sessionScope.loginuser.fk_department_id}" name="fk_department_id"/>
		<input type="hidden" value="${sessionScope.loginuser.email}" name="fk_email"/>
	</form>
	
	<div style="float: right;">
	<button type="button" id="register" class="btn_normal" style="margin-right: 10px; background-color:rgb(3,199,90);">등록</button>
	<button type="button" class="btn_normal" style="background-color: #990000;" onclick="javascript:location.href='<%= ctxPath%>/schedule/scheduleManagement.gw'">취소</button> 
	</div>
</div>
</div>
