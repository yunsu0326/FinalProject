	 ...
	 //	console.log(end_hour);
		...
		$.ajax({
				...
				data: {"fk_lgcatgono":"${requestScope.map.FK_LGCATGONO}", 
					   "fk_employee_id":"${requestScope.map.FK_EMPLOYEE_ID}",
						...
						if(json.length > 0){
							...
							$.each(json, function(index,item){
								...
								if(name.includes(joinUserName)){ // name 이라는 문자열에 joinUserName 라는 문자열이 포함된 경우라면 true , 
								   ...
								   joinUserArr.push(name+"("+item.employee_id+")");
								}
							});
		...
		//	console.log("공유자 => " + joinuser);
		    ...
		    var frm = document.scheduleFrm;
		  	frm.action="<%= ctxPath%>/schedule/editSchedule_end.gw";
			frm.method="post";
			frm.submit(); 
...
<h3 style="display: inline-block;">일정 수정하기</h3>&nbsp;&nbsp;<a href="<%= ctxPath%>/schedule/scheduleManagement.gw"><span>◀캘린더로 돌아가기</span></a>
	...
	<button type="button" id="edit" class="btn_normal" style="margin-right: 10px; background-color:rgb(3,199,90);">완료</button>
