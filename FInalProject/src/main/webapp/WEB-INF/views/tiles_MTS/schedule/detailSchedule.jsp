...
<script type="text/javascript">
	...
	function delSchedule(scheduleno){
		...
		if(bool){
			$.ajax({
				url: "<%= ctxPath%>/schedule/deleteSchedule.gw",
				type: "post",
				...
				success:function(json){
					...
					location.href="<%= ctxPath%>/schedule/scheduleManagement.gw";
				},
	...
	function editSchedule(scheduleno){
		...
		frm.action = "<%= ctxPath%>/schedule/editSchedule.gw";
		frm.method = "post";
		frm.submit();
	}
		...
		<table id="schedule" class="table table-bordered">
			...
			<tr>
				...
				<c:if test="${requestScope.map.FK_LGCATGONO eq '2'}">
					회사 캘린더 - ${requestScope.map.SMCATGONAME}
				</c:if>
				<c:if test="${requestScope.map.FK_LGCATGONO eq '1'}">
					개인 캘린더 - ${requestScope.map.SMCATGONAME}
				</c:if>
				<c:if test="${requestScope.map.FK_LGCATGONO eq '3'}">
					부서 캘린더 - ${requestScope.map.SMCATGONAME}
				</c:if></td>
			</tr>
			<c:if test="${requestScope.map.FK_LGCATGONO eq '3'}">
			<tr>
				<th style="vertical-align: middle;">부서</th>
				<td>${requestScope.map.DEPARTMENT_NAME}</td>
			</tr>
			</c:if>
			<tr>
	...
	<input type="hidden" value="${sessionScope.loginuser.employee_id}" />
	<input type="hidden" value="${requestScope.map.FK_LGCATGONO}" />
	
	<c:set var="v_fk_employee_id" value="${requestScope.map.FK_EMPLOYEE_ID}" />
	<c:set var="v_fk_lgcatgono" value="${requestScope.map.FK_LGCATGONO}"/>
	<c:set var="v_loginuser_employee_id" value="${sessionScope.loginuser.employee_id}"/>
	
	<c:set var="v_fk_department_id" value="${requestScope.map.FK_DEPARTMENT_ID}" />
	<c:set var="v_loginuser_fk_department_id" value="${sessionScope.loginuser.fk_department_id}"/>
	...
	<div style="float: right;">
	    ...
	    --%>
			<c:if test="${v_fk_lgcatgono eq '2' && sessionScope.loginuser.gradelevel == 10 }">
				<button type="button" id="edit" class="btn_normal" style="background-color:rgb(3,199,90);" onclick="editSchedule('${requestScope.map.SCHEDULENO}')">수정</button>
				<button type="button" class="btn_normal" onclick="delSchedule('${requestScope.map.SCHEDULENO}')">삭제</button>
			</c:if>
			<c:if test="${v_fk_lgcatgono eq '1' && v_fk_employee_id eq v_loginuser_employee_id}">
				<button type="button" id="edit" class="btn_normal" style="background-color:rgb(3,199,90);" onclick="editSchedule('${requestScope.map.SCHEDULENO}')">수정</button>
				<button type="button" class="btn_normal" onclick="delSchedule('${requestScope.map.SCHEDULENO}')">삭제</button>
			</c:if>
			<c:if test="${v_fk_lgcatgono eq '3' && v_fk_department_id eq v_loginuser_fk_department_id}">
				<button type="button" id="edit" class="btn_normal" style="background-color:rgb(3,199,90);" onclick="editSchedule('${requestScope.map.SCHEDULENO}')">수정</button>
				<button type="button" class="btn_normal" onclick="delSchedule('${requestScope.map.SCHEDULENO}')">삭제</button>
			</c:if>
	    ...
	    --%>
			...
			<c:if test="${(v_fk_lgcatgono eq '3' && v_fk_department_id eq v_loginuser_fk_department_id) }">
				...
				<button type="button" id="cancel" class="btn_normal" style="margin-right: 0px; background-color: #990000;" onclick="javascript:location.href='<%= ctxPath%>/schedule/detailSchedule.gw'">취소</button> 
		</c:if>
...
<form name="goEditFrm">
	...
	<input type="hidden" name="fk_lgcatgono" value="${requestScope.map.FK_LGCATGONO}"/>
	<input type="hidden" name="fk_department_id" value="${requestScope.map.FK_DEPARTMENT_ID}"/>
